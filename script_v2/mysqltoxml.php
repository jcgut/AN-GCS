...
mysqli_query($conn, "ALTER TABLE ". $argv[1].".unidades_descriptivas_h2 RENAME COLUMN `Descrip. Onomasticos` TO DescripOnomasticos;");
mysqli_query($conn, "ALTER TABLE ". $argv[1].".unidades_descriptivas_h2 RENAME COLUMN `Descrip. Asuntos` TO DescripAsuntos;");
mysqli_query($conn, "ALTER TABLE ". $argv[1].".unidades_descriptivas_h2 RENAME COLUMN `Descrip. Geograficos` TO DescripGeograficos;");
$result = mysqli_query($conn, "SELECT * from $argv[1].unidades_descriptivas_h2 Where Id_Registro > $argv[2]");
sqlToXml($result);
...
function sqlToXml($queryResult){
	...
	while($record = mysqli_fetch_object($queryResult)){ 
			...
			if($invalid1 != "fallo" && $invalid2 != "fallo" && $invalid3 != "fallo"){
				$myfile = fopen('files/file-'.$archivos.'.xml', 'w') or die('Unable to open file!');
